...
while True:
    ...
    for index, ship in enumerate(me.get_ships()):
        try:
            ...
            nearby_ships = get_nearby_ships()

            if game.turn_number >= constants.MAX_TURNS/1.05 and not ship_position.has_structure:
                command = go_drop_final()
            elif nearby_ships is 4 and bank > constants.DROPOFF_COST and not ship_position.has_structure:
                command = make_drop()
            elif nearby_ships < 3 and ship.halite_amount >= 750 - cost_to_move and not ship_position.has_structure:
                command = go_drop()
            elif cost_to_move <= ship.halite_amount and not ship_position.has_structure:
